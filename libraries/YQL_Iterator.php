 ...
 * @package  YQL
 * @author   Sam Clark
 * @version  $Id: YQL_Iterator.php 9 2009-05-14 10:24:53Z samsoir $
...
class YQL_Iterator_Core implements Iterator, ArrayAccess, Countable
{
	...
	{
		if ($this->offsetExists($key) AND is_array($this->result[$key]))
		{
			if (is_int(key($this->result[$key])))
				return new YQL_Iterator($this->result[$key]);
			else
